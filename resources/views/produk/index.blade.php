                ...
                <div class="box-header with-border">
                    <div class="btn-group">
                        ...
                        <button onclick="deleteSelected('{{ route('produk.delete_selected') }}')"
                            class="btn btn-danger xs btn-flat"><i class="fa fa-trash"></i>Hapus</button>
                        <button onclick="cetakBarcode('{{ route('produk.cetak_barcode') }}')"
                            class="btn btn-info xs btn-flat"><i class="fa fa-barcode"></i>Cetak Barcode</button>
                    </div>
    ...
    <script>
        ...
        function deleteSelected() {
            if ($('input:checked').length > 1) {
                if (confirm('Yakin ingin menghapus data terpilih?')) {
                    $.post(url, $('.form_produk').serialize())
                        .done((response) => {
                            table.ajax.reload();
                        })
                        .fail((errors) => {
